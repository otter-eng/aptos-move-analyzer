...
impl Default for FmtConfig {
    fn default() -> Self {
        Self {
            enable: true, // Enable formatting by default
            max_width: 90,
            indent_size: 4,
        }
    }
...
pub fn on_movefmt_request(
    ...
    if !fmt_cfg.enable {
        log::info!("movefmt disabled.");
        return Response::new_err(request.id.clone(), -1, "movefmt disabled.".to_string());
    }

    let parameters = serde_json::from_value::<DocumentFormattingParams>(request.params.clone())
        .expect("could not deserialize DocumentFormattingParams");
    ...
    let project = match context.projects.get_project(&fpath) {
        ...
        None => {
            ...
            return Response::new_err(request.id.clone(), -1, "No available project".to_string());
        }
    };
    ...
    log::info!("File content length: {} characters", content_origin.len());
    ...
    let content_format = movefmt::core::fmt::format_entry(content_origin.clone(), movefmt_cfg);
    log::info!(
        "Format result: {:?}",
        content_format.as_ref().map(|s| s.len())
    );
    ...
    log::info!(
        "Format successful, formatted content length: {}",
        content_format.len()
    );

    let mut text_edits = vec![];

    let before_line = content_origin.lines().count();
    let after_line = content_format.lines().count();
